import pytest
import time
from .scenarios import HAPPY_PATH_SCENARIOS, GSB_LEAD_SCENARIOS
...
@pytest.mark.parametrize("scenario", GSB_LEAD_SCENARIOS, ids=lambda s: s["test_id"])
def test_gsb_lead_workflow(session, email, password, scenario):
    ...
    loan_status_found = False
    for item in customer_data_list:
        if item.get("field_name") == "_thinker.loanStatus" and item.get("value") == "PRE-APPROVED":
            loan_status_found = True
            break
    assert loan_status_found, "Expected '_thinker.loanStatus' with 'PRE-APPROVED' value not found in customer_data"
    ...
    loan_status_found = False
    for item in customer_data_list:
        if item.get("field_name") == "_thinker.loanStatus" and item.get("value") == "APPROVED":
            loan_status_found = True
            break
    assert loan_status_found, "Expected '_thinker.loanStatus' with 'APPROVED' value not found in customer_data"
    ...
    loan_status_found = False
    for item in customer_data_list:
        if item.get("field_name") == "_thinker.loanStatus" and item.get("value") == "COMPLETED":
            loan_status_found = True
            break
    assert loan_status_found, "Expected '_thinker.loanStatus' with 'COMPLETED' value not found in customer_data"
